...
public class ScorpionSpawnItem extends Item {

    public ScorpionSpawnItem(Settings settings) {
        super(settings);
    }
        ...
        if(!context.getWorld().isClient()) {
            ...
            for(int i = 0; i <= positionClicked.getY() + 2; i++) {
                ...
                if(isValuableBlock(blockBelow)) {
                    ...
                    ScorpionEntity spawnAngel = EntityTypes.SCORPION.create(world);
                    spawnAngel.refreshPositionAndAngles((double)positionClicked.getX() + 0.5D, (double)positionClicked.getY()
                            + 2.05D, (double)positionClicked.getZ() + 0.5D, 0.0F, 0.0F);
